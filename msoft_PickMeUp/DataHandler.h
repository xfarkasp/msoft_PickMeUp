...
struct Parcel
{
	Parcel(int32_t id, std::string sender, std::string reviever, std::string sAddress, std::string dAddress)
		:m_parcelId(id), m_sender(sender), m_reciever(reviever), m_sourceAddress(sAddress), m_destinationAddress(dAddress), m_status(ParcelStatus::Created)
	{}
...
public:
	...
	void writteParcel(const std::string& sender, const std::string& reviever, const std::string& sAddress, const std::string& dAddress)
	{
		m_parcels.emplace(m_currentParcelId, Parcel{ m_currentParcelId , sender, reviever, sAddress,  dAddress});
		m_currentParcelId++;
	}
